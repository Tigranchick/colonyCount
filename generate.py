...
cur_dirs = ['train', 'validate']
file_basename = 'colony'
file_ext = 'png'

if os.path.exists(os.path.join(base_dir, 'data')):
    response = response = input('The path given already exists. Would you like to delete it? [y/n] ')
...
for dataset_size_index, cur_dir in enumerate(cur_dirs, 0):
    ...
    with open(os.path.join(base_dir, cur_dir, 'labels.csv'), 'w') as csv_file:
